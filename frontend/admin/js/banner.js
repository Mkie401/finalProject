...
async function loadAllBanners() {
    ...
    try {
        ...
        // console.log('從後端載入', banners.length, '筆輪播圖');
    } catch (error) {
...
function displayImage(bannerItem, selector, imageData, title) {
    ...
    try {
        ...
        if (imageData.startsWith('data:image/')) {
            ...
            // console.warn('無法識別的圖片格式，長度:', imageData.length, '內容:', imageData);
            return;
        }
        ...
        if (imageSrc) {
            ...
            // console.log(`成功顯示圖片 - selector: ${selector}, 資料長度: ${imageData.length}`);
        }
    } catch (error) {
...
async function handleUpdateBanner(bannerItem, bannerId, button) {
    ...
    try {
        ...
        // console.log('更新成功，返回資料:', result);
        // console.log('bannerLg 長度:', result.bannerLg?.length || 0);
        // console.log('bannerSm 長度:', result.bannerSm?.length || 0);
