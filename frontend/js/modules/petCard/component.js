                ...
                <div class="card shadow" data-casenumber="${data.caseNumber}" data-id="${data.id}">
                    <div class="card-header-customer px-3 pt-3">
                        <div class="card-img-wrapper">
                            <img src="${data.photo ? 'data:image/png;base64,' + data.photo : 'https://www.pet.gov.tw/upload/pic/1760670698786.png'}" class="card-img-customer">
                        </div>
                        <div>
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col ms-auto text-end">
                                <span class="card-title fw-bold d-inline-block text-truncate">${data.petName}</span>
                            </div>
                        </div>
                        ...
                        <p class="card-text position-relative">
                            ${
                                isAdoption ? 
                                    (
                                        (data.isOpen ? '<span class="pet-status fw-bold">開放認養中</span>' : '<span class="pet-status fw-bold status-match">領養媒合中</span>') +
                                            '<strong>寵物物種: ' + data.species + '</strong><br>' +
                                            '<strong>寵物品種: ' + data.breed + '</strong><br>' +
                                            '<strong>寵物體型: ' + (sizeMap[data.size] || data.size) + '</strong><br>' +
                                            '<strong>所在地區: ' + data.region + '</strong><br></br>'
                                    ) : (
                                         '<strong>走失時間: ' + formatDate(data.lostDate) + '</strong><br>' +
                                        '<strong>走失地: ' + data.lostRegion + '</strong><br>' +
                                        '<strong>寵物物種: ' + data.species + '</strong><br>' +
                                        '<strong>寵物品種: ' + data.breed + '</strong><br></br>'
                                    )
                            }
                        </p>
                        <p>
...
export function renderDetail(parsedData) {
    ...
    // main image
    $('#detail_main_img').prop('src', (parsedData.images.length ? 'data:image/png;base64,' : '') + images[0]);
    if (images.length > 1) {
        // 如果有 carousel，填入其餘圖片
        let imgHtml = '';
            ...
            imgHtml += `
                <li class="page-item col m-1">
                    <div class="img-wrapper my-4">
                        <img src="data:image/png;base64, ${images[i]}" class="img-customer">
                    </div>
                </li>
            `;
        }
    ...
    if (parsedData.isMissing) {
        descriptionHtml += `
            <div class="description-body">
                ...
                <div class="description-content p-4">
                    <span>${parsedData.lostDetail.lostProcess}</span>
                </div>
            </div>
...
export function bindSearchResetAndTagHandlers(dataTypeList, callbacks = {}) {
    ...
    // reset
    $('#adoption_search .dropdown-group .reset-btn').on('click', function (e) {
        e.preventDefault();
    ...
    // search
    $('#adoption_search .dropdown-group .search-btn').on('click', function (e) {
        e.preventDefault();
...
export function getSearchFormAndParse(pageNumber = 1, isAdoption = true) {
    ...
    const limit = getSingleSelect('#search_criteria_page_limit', '15');
