...
function restoreFormData(data) {
    ...
    if (data.petNeutered) document.getElementById('pet-neutered').value = data.petNeutered ? 'true' : 'false';
    if (data.petMicrochip) document.getElementById('pet-microchip').value = data.petMicrochip ? 'true' : 'false';
    ...
    if (data.petLocationId) {
        ...
        if (data.petDistrictId) {
            loadDistricts(Number(data.petLocationId), Number(data.petDistrictId));
        }
    }
    ...
    if (data.phoneDisplay) document.getElementById('phone-display').value = data.phoneDisplay ? 'true' : 'false';
    if (data.contactEmail) document.getElementById('contact-email').value = data.contactEmail;
    if (data.emailDisplay) document.getElementById('email-display').value = data.emailDisplay ? 'true' : 'false';
...
async function loadDistricts(cityId, selectedDistrictId) {
    ...
    const resp = await fetch(`/api/se/regions?cityId=${cityId}`);
    const result = await resp.json();
...
// 初始化縣市與區域選單
function initLocationSelectors(defaultCityId, defaultDistrictId) {
    // 直接載入所有縣市選項
    loadCity(defaultCityId);

    // 如果有預設的縣市ID，載入對應的區域
    if (defaultCityId) {
        loadDistricts(defaultCityId, defaultDistrictId);
    }
